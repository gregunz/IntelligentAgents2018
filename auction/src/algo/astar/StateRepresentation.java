...
public class StateRepresentation implements State {
    ...
    public StateRepresentation(Vehicle vehicle, List<Task> tasks) {
        this(vehicle.getCurrentCity(), new ArrayList<>(vehicle.getCurrentTasks()), vehicle.capacity(), tasks, vehicle.costPerKm());
    }
    ...
    @Override
    public VehiclePlan toVehiclePlan(Vehicle v) {
        VehiclePlan vehiclePlan = new VehiclePlan(v);
        previousActions.forEach(action -> {
            if (action.isPickup()) {
                vehiclePlan.addLoadAction(action.getTask().get());
            } else if (action.isDeliver()) {
                vehiclePlan.addDropAction(action.getTask().get());
            }
        });
        return vehiclePlan;
    }
